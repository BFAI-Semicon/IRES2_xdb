...
- **In**:
  - ユーザーのグループ所属
  - 試料の作成・編集（作成者およびその試料を使ったユーザー、訂正履歴、試料管理者、参考 URL・タグ・試料の説明）
  - 装置台帳（製造装置／検査装置、主用途で 1 種別、台帳はシステムに 1 つ、データファイルの拡張子）
  - 固有情報欄の定義（装置担当ロール。全装置の欄を定義可。装置に担当を紐づけない）
  - 実験データの登録・訂正・履歴（必須: ユーザー・装置・試料 1 つ・実施日時。任意: 固有情報・データファイル・添付・説明・データ所有者）
  - ユーザーの任意項目（DICE ID・所属・機関メールアドレス）。送出時の後追い入力を避けるための器
  - Web 手入力 UI
- **Out**:
  - ARIM RDE 送出、課題・データセット・送出記録、OCR、API
  - 予約・見える化・映像化・解析
  - 工程エンティティ、施設概念

## Boundary Candidates

- 実験データ（登録単位・訂正・データファイル・添付・データ所有者）
- 試料・グループ（試料管理者）
- 装置台帳と固有情報スキーマ
- ユーザー／装置担当／グループ担当ロールと権限（グループの役割はこの 2 つに限る）

## Out of Boundary

- ARIM RDE の課題・データセット・データ名・実験 ID の送出マッピング、DICE ID の解決、送出記録
- OCR／カメラ入力、外部 API 受付
- 装置予約・稼働見える化
...
- 1 実験データは試料を 1 つだけ指す（ARIM RDE の 1 データ = 1 試料に合わせる）
- 固有情報・データファイル・添付・説明は空でも登録可
- DICE ID を持つのはユーザーのみ。実験データ・試料には持たせない
